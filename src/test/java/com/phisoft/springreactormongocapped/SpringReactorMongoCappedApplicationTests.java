package com.phisoft.springreactormongocapped;

import com.phisoft.springreactormongocapped.controller.EmployeeController;
...
class SpringReactorMongoCappedApplicationTests {
    ...
    void streamEmployees() {
        Flux<EmployeeDto> employeeDtoFlux=Flux.just(new EmployeeDto("a2b3","Tony",20000L),
                new EmployeeDto("a1b4","Peter",30000L),
                new EmployeeDto("a3b2","Larry",40000L));
        when(employeeService.getEmployees()).thenReturn(employeeDtoFlux.delayElements(Duration.ofSeconds(1)));
        Flux<EmployeeDto> responseBody = webTestClient.get().uri("/employee/stream").
                exchange()
                .expectStatus()
                .isOk()
                .returnResult(EmployeeDto.class)
